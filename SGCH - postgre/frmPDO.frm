...
'            gravaLog "Código req: " & txtCadReq(0), "Requisitante" & txtCadReq(1) & "-" & txtCadReq(2), ""
        End If
    Case 12
        If MsgBox("Deseja sair da tela de avaliação do PDO?", vbQuestion + vbYesNo, "SGCH") = vbYes Then
            Unload Me
            Set frmPDO = Nothing
        End If
    End Select
...
Private Sub Form_Activate()
    If vAvaliacao = False Then
        MsgBox "Essa solicitação se encontrar processada pelo solicitante. Não pode ser alterada"
        Unload Me
    End If
End Sub
...
Private Function dadosEmail()
    ...
    If vEmailSolicitante = "" Then
        MsgBox "Email do usuário SOLICITANTE não está cadastrado"
        Exit Function
    End If
    rsEnviaEmail.Close
...
Private Function ValidaCampos()
    ...
    If Option1.Value = False And Option2.Value = False Then
        MsgBox "Favor informar o campo " & Option1.Tag, vbCritical, "Atenção"
