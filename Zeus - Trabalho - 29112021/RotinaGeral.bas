...
Public vOpenTabs As String ' Informa o significado (F)Fone (F) Fax (C) Celular
...
Public Sub CompoeComboLV(Combo As ComboBox, Optional Column As ColumnHeader = Nothing)
    ...
    'If Column Is Nothing Then
        For Each c In vListViewPrincipal.ColumnHeaders
            Combo.AddItem c
        Next
...
'    oConn.Execute "CREATE TABLE " & sDatabaseName & ".dbo.TBIMPOSTOSSERVICOS(" & _
'    "id INT NOT NULL," & _
'    "nome VARCHAR(50) NOT NULL," & _
'    "aliquota decimal(10,2) NOT NULL," & _
'    "tipo VARCHAR(100) NOT NULL," & _
'    "descricao TEXT NULL," & _
'    "formula VARCHAR(200) NOT NULL," & _
'    "PRIMARY KEY (id))"
    
    oConn.Execute "CREATE TABLE " & sDatabaseName & ".dbo.TBIMPOSTOSSERVICOS(" & _
    "IDIMPOSTOSSERVICOS INT NOT NULL IDENTITY," & _
    "NMIMPOSTOSSERVICOS VARCHAR(100) NOT NULL," & _
    "TIPO INT NOT NULL CHECK(TIPO IN(1,2))," & _
    "DESCRICAO TEXT NULL," & _
    "F_ALIQUOTA VARCHAR(300) NULL," & _
    "F_VALOR VARCHAR(300) NULL," & _
    "F_VALORKG VARCHAR(300) NULL," & _
...
''FUNCTION FOI SUBSTITUIDA PELA FUNCTION ABAIXO, QUE REALIZA O REDIMENSIONAMENTO DOS COMPONENTES CRIADOS DINAMICAMENTE
...
Public Function DimensionaLV1(NomeLV As String, vFrame As Frame, vListview As Listview, vLabel As Label)
    frmPesqGeralTeste2.Move 0, 0, Principal.ScaleWidth - 10, Principal.ScaleHeight - 50
    'MeuLV.Frame1.Caption = NomeLV
    frmPesqGeralTeste2.SSTab1.Move 0, 0, Principal.ScaleWidth - 430, Principal.ScaleHeight - 850
    
    vLabel.Move 110, Principal.ScaleHeight / 2, Principal.ScaleWidth - 1000, 400
    vFrame.Move 100, 400, Principal.ScaleWidth - 700, Principal.ScaleHeight - 1430
    vListview.Move 240, 1400, Principal.ScaleWidth - 1000, Principal.ScaleHeight - 2600
End Function
...
Public Function MudaPropPictureTeste(vIndex As Integer, vListview As Listview)
On Error GoTo Err
    frmPesqGeralTeste2.picBg(vIndex).BackColor = vListview.BackColor
    frmPesqGeralTeste2.picBg(vIndex).ScaleMode = vbTwips
    frmPesqGeralTeste2.picBg(vIndex).BorderStyle = vbBSNone
    frmPesqGeralTeste2.picBg(vIndex).AutoRedraw = True
    frmPesqGeralTeste2.picBg(vIndex).Visible = False
    Exit Function
Err:
    If Err.Number = 91 Then
    ElseIf Err.Number = 340 Then
    Else
        Msgbox Err.Number & " - " & Err.Description, vbCritical, "Zeus - ERRO"
        Exit Function
    End If
    Resume Next
End Function
...
Public Function MudaCorListTeste(vListview As Listview, vIndex As Integer)
    Dim i As Integer
    frmPesqGeralTeste2.picBg(vIndex).Width = vListview.Width
    frmPesqGeralTeste2.picBg(vIndex).Height = vListview.ListItems(1).Height * (vListview.ListItems.Count)
    frmPesqGeralTeste2.picBg(vIndex).ScaleHeight = vListview.ListItems.Count
    frmPesqGeralTeste2.picBg(vIndex).ScaleWidth = 1
    frmPesqGeralTeste2.picBg(vIndex).DrawWidth = 1
    frmPesqGeralTeste2.picBg(vIndex).Cls
    frmPesqGeralTeste2.BackColor = &H80000018
    For i = 1 To 5000
        localizaCorTipoFCE Trim(vGuardaLinhaTipo(i, 0))
        If vIDCorTipoFCE <> "" Then frmPesqGeralTeste2.picBg(vIndex).Line (0, vGuardaLinhaTipo(i, 1) - 1)-(1, vGuardaLinhaTipo(i, 1)), vIDCorTipoFCE, BF
    Next
    vListview.Picture = frmPesqGeralTeste2.picBg(vIndex).Image
    vListview.Refresh
End Function
...
Public Function PersonaColLVTeste(posCol As Integer, negritoCol As String, corCol As String, caracterCol As String, imageCol As String, formataColZero As String, formataColDecimal As String, alinhaCol As String, vListview As Listview)
    ...
    For X = 1 To Y
        ...
        'ALINHAMENTO DA COLUNA
        If alinhaCol = "D" Then
            vListview.ColumnHeaders(posCol + 1).Alignment = lvwColumnRight
        ElseIf alinhaCol = "E" Then
            vListview.ColumnHeaders(posCol + 1).Alignment = lvwColumnLeft
        Else
            vListview.ColumnHeaders(posCol + 1).Alignment = lvwColumnCenter
        End If
    Next
    
    If corCol = "P" Then
        MudaCorListTeste vListview, frmPesqGeralTeste2.SSTab1.Tab
    End If
    limpaGuardaLinhaTipo
    Principal.ProgressBar1.Value = 0
    Legenda = ""
...
On Error GoTo Err
    ...
    If LV.ListItems.Count > 0 Then
        ...
        Next
    End If
    If vRaptor(1) <> "" Then vCP01.Text = vCP01.Text + vRaptor(1)
    Exit Function
Err:
    Resume Next
End Function
...
''FUNCTION DESATIVADA POR TER NECESSIDADE NA CRIAÇÃO DO COMPONENTES DINÂMICOS
