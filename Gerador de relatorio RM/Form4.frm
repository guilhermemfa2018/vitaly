...
Begin VB.Form Form4 
   Caption         =   "Resumo de produtos por FCE"
   ClientHeight    =   3195
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   4680
...
Private Sub Form_Load()
    ...
    If vMovimento = "1.1.03" Then
        sqlRelAvTrei = "SELECT '" & vNomeColigada & "' AS NOMECOLIGADA,TMOV.NUMEROMOV AS NUMNF,FCFO.NOMEFANTASIA AS FORNECEDOR,TPRD.NOMEFANTASIA AS PRODUTO,TPRD.CODIGOPRD AS CODPRODUTO,TPRD.CODUNDCONTROLE AS UND,TITMMOV.QUANTIDADETOTAL AS QTDE,TITMMOV.QUANTIDADEARECEBER as QTDAREC,TITMMOV.QUANTIDADETOTAL - TITMMOV.QUANTIDADEARECEBER as QTDRECEBIDA,TITMMOV.PRECOUNITARIO AS PRECOUNIT,((TITMMOV.QUANTIDADETOTAL-TITMMOV.QUANTIDADEARECEBER)*TITMMOV.PRECOUNITARIO) AS TOTALITENS,TCPG.NOME AS CONDPAG,DATEPART(YEAR,TMOV.DATAEMISSAO)AS ANO,DATEPART(MONTH,TMOV.DATAEMISSAO)AS MES,DATEPART(DAY,TMOV.DATAEMISSAO)AS DIA,TMOV.CODTMV AS TPMOV," & _
                     "TMOV.CAMPOLIVRE3 AS FCE,TMOV.VALORBRUTO AS VLRPROD,TMOV.VALORLIQUIDO AS VLRLIQUIDO,TMOV.IDMOV,TMOV.status,TITMMOVCOMPL.LARGURA,TITMMOVCOMPL.COMPR, " & _
                     "SC=(SELECT DISTINCT SUBSTRING(( SELECT '/' + convert(varchar(15),T2.NUMEROMOV) AS [text()] FROM TITMMOVRELAC A2 LEFT JOIN TMOV AS T2 ON A2.IDMOVORIGEM = T2.IDMOV AND T2.CODCOLIGADA = " & vCodColigada & " " & _
                     "WHERE A2.IDMOVDESTINO = A1.IDMOVDESTINO AND A2.CODCOLDESTINO = A1.CODCOLDESTINO GROUP BY T2.NUMEROMOV FOR XML PATH('')), 2, 1000) AS SC FROM TITMMOVRELAC A1 LEFT JOIN TMOV AS T ON " & _
                     "A1.IDMOVDESTINO = T.IDMOV AND T.CODCOLIGADA = " & vCodColigada & " WHERE IDMOVDESTINO = TMOV.IDMOV AND CODCOLDESTINO = " & vCodColigada & ") " & _
                     "FROM TMOV,FCFO,TITMMOV,TPRD,TCPG,TITMMOVCOMPL WHERE CODTMV IN('" & vMovimento & "')AND TMOV.CODCOLIGADA=FCFO.CODCOLIGADA AND TMOV.CODCFO=FCFO.CODCFO AND TMOV.CODCOLIGADA=TITMMOV.CODCOLIGADA AND TMOV.IDMOV=TITMMOV.IDMOV AND TITMMOV.IDPRD=TPRD.IDPRD AND TITMMOV.CODCOLIGADA=TPRD.CODCOLIGADA AND TITMMOV.IDMOV = TITMMOVCOMPL.IDMOV AND TITMMOV.NSEQITMMOV = TITMMOVCOMPL.NSEQITMMOV AND TMOV.DATAEMISSAO BETWEEN '" & Format(vDataFilter1, "yyyy/mm/dd") & "' and '" & Format(vDataFilter2, "yyyy/mm/dd") & "' AND " & _
                     "TMOV.CODCPG=TCPG.CODCPG AND TMOV.SERIE='OC' AND TMOV.STATUS IN ('F','G','A') AND TPRD.NOMEFANTASIA LIKE '" & "%" & vProduto & "%" & "' AND TMOV.CAMPOLIVRE3 LIKE '" & "%" & vFCE & "%" & "' AND TMOV.CODCOLIGADA IN(" & vCodColigada & ") and FCFO.CODCOLIGADA IN(" & vCodColigada & ") and TITMMOV.CODCOLIGADA IN(" & vCodColigada & ") and tprd.CODCOLIGADA IN(" & vCodColigada & ") and  tcpg.CODCOLIGADA IN(" & vCodColigada & ") and  TITMMOVCOMPL.CODCOLIGADA IN(" & vCodColigada & ") ORDER BY ANO, PRODUTO"
    End If
    If vMovimento = "1.1.02" Then
        sqlRelAvTrei = "select  '" & vNomeColigada & "'  AS NOMECOLIGADA,a.NUMEROMOV AS NUMNF,b.NOMEFANTASIA AS FORNECEDOR,d.NOMEFANTASIA AS PRODUTO,d.CODIGOPRD AS CODPRODUTO,d.CODUNDCONTROLE AS UND,c.QUANTIDADETOTAL AS QTDE,c.QUANTIDADEARECEBER as QTDAREC,c.QUANTIDADETOTAL - c.QUANTIDADEARECEBER as QTDRECEBIDA,c.PRECOUNITARIO AS PRECOUNIT,((c.QUANTIDADETOTAL-c.QUANTIDADEARECEBER)*c.PRECOUNITARIO) AS TOTALITENS,e.NOME AS CONDPAG,DATEPART(YEAR,a.DATAEMISSAO)AS ANO," & _
                     "DATEPART(MONTH,a.DATAEMISSAO)AS MES,DATEPART(DAY,a.DATAEMISSAO)AS DIA,a.CODTMV AS TPMOV,a.CAMPOLIVRE3 AS FCE,a.VALORBRUTO AS VLRPROD,a.VALORLIQUIDO AS VLRLIQUIDO,a.IDMOV,a.status,a.status,f.LARGURA,f.COMPR from TMOV as a left join FCFO as b on a.CODCOLIGADA=b.CODCOLIGADA AND a.CODCFO=b.CODCFO left join TITMMOV as c " & _
                     "on a.CODCOLIGADA=c.CODCOLIGADA AND a.IDMOV=c.IDMOV left join TPRD as d on c.IDPRD=d.IDPRD AND c.CODCOLIGADA=d.CODCOLIGADA left join TCPG AS e on a.CODCPG=e.CODCPG left join TITMMOVCOMPL as f on c.IDMOV = f.IDMOV and c.NSEQITMMOV = f.NSEQITMMOV where a.CODTMV IN('1.1.02') AND a.SERIE='SC' AND a.DATAEMISSAO BETWEEN '" & Format(vDataFilter1, "yyyy/mm/dd") & "' and '" & Format(vDataFilter2, "yyyy/mm/dd") & "' AND d.NOMEFANTASIA LIKE '" & "%" & vProduto & "%" & "' AND a.CAMPOLIVRE3 LIKE '" & "%" & vFCE & "%" & "' AND A.CODCOLIGADA IN(" & vCodColigada & ") and B.CODCOLIGADA IN(" & vCodColigada & ") and C.CODCOLIGADA IN(" & vCodColigada & ") and D.CODCOLIGADA IN(" & vCodColigada & ") and E.CODCOLIGADA IN(" & vCodColigada & ") and  F.CODCOLIGADA IN(" & vCodColigada & ") ORDER BY ANO, PRODUTO"
    End If
