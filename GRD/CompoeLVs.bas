...
Public Sub MontaLV(QualLV As Integer)
        ...
        If QualLV = 0 Then
            ...
            If Pesquisa <> "excluir" And Pesquisa <> "novo" And Pesquisa <> "editar" And Pesquisa <> "0" Then
                ...
                If FiltroGeral = "" Then
                    frmFiltro.Show 1
                    If Pesquisa = "CANCELAR" Then Exit Sub
                Else
                    filtroPadrao
                    LocalString1 SqlLV
...
'                                 "INNER JOIN tbDevolucaoItens AS B ON A.idmov = B.idmov where a.localestoque =  " & Val(vLocalEstoque) & "  group by a.chapa,a.nome,a.codfuncao,a.nomefuncao,a.codsecao,a.nomesecao,B.datadevolucao,a.idmov,a.numeromov,a.serie,B.nomequememprestou,a.codusuariorm,c.CODSITUACAO,b.idmovemp order by b.datadevolucao desc,a.idmov desc"
            ...
            QtdColReal = 0
            MontaCabLV "Cód Medição", "Empresa", "Executante", "Seção", "Período", "Projeto", "Competência", "Valor Total", "D.Cad.", "NF", "Status", "CNPJ", "Filial", "ID Fornec", "Status Env.", "NF Totvs", "Tipo Mov.", "Ap Prim.", "Dt. Ap. Prim.", "Ap. Secun.", "Dt. Ap. Secun", "", "", "", "", ""
...
'                                 "INNER JOIN tbDevolucaoItens AS B ON A.idmov = B.idmov where a.localestoque =  " & Val(vLocalEstoque) & "  group by a.chapa,a.nome,a.codfuncao,a.nomefuncao,a.codsecao,a.nomesecao,B.datadevolucao,a.idmov,a.numeromov,a.serie,B.nomequememprestou,a.codusuariorm,c.CODSITUACAO,b.idmovemp order by b.datadevolucao desc,a.idmov desc"
            ...
            MeuLV.Visible = True
            Exit Sub
        'CADASTRO DAS PREVISÕES FINANCEIRAS
        ElseIf QualLV = 2 Then
            ...
            'Set chamaForm = New frmEmprestimo
            Formulario = "Previsões Financeiras"
            LegendaExc = "Previsão Financeira" 'Usado na mensagem de exclusão
            indiceVarGlobal = 1
            If Pesquisa <> "excluir" And Pesquisa <> "novo" And Pesquisa <> "editar" And Pesquisa <> "0" Then
                MontaFiltro
                If FiltroGeral = "" Then
                ...
                If MeuLV.Visible = True Then
                    Unload MeuLV
                End If
                carregaTABS "ZMDDPPREVFIN", "FTB1", "", "", "", "", "", "", "", "", "", "", "", "", ""
                If FiltroGeral = "Todos" Then SqlLV = "SELECT top " & LimiteLinhas & " A.ID,A.CODNATFIN,B.DESCRICAO,CASE WHEN A.PAGREC = 1 THEN 'Receita' WHEN A.PAGREC = 2 THEN 'Despesa' END as PAGREC,A.VALOR,CONVERT(VARCHAR(10), A.DTVENCIMENTO, 103) AS DTVENCIMENTO,A.OBS FROM " & vBancoSAP & ".DBO.ZMDDPPREVFIN AS A INNER JOIN " & vBancoSAP & ".DBO.FTB1 AS B ON A.CODNATFIN = B.CODTB1FLX and b.CODCOLIGADA = " & vCodcoligada & " order by ID"
            Else
                If MeuLV.Visible = True Then Unload MeuLV
            End If
            ...
            MeuLV.cmdconsulta(0).Visible = False
            QtdColReal = 0
            MontaCabLV "ID", "Nat. Fin.", "Descrição", "Tipo", "Valor", "DT. Vencimento", "Observação", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""
            DimensionaLV "Previsão Financeira"
            MontaCabecalhoLV
            MontaDadosLV "S"
            If checaFiltro = True Then
                'PersonaColLV 11, "N", "N", "", "N", "N", "S", "D"
                PersonaColLV 4, "N", "N", "", "N", "N", "S", "D"
                'PersonaColLV 16, "N", "N", "", "S", "N", "N", "E"
            End If
            If MeuLV.ListView1.ListItems.Count > 0 Then ajusta_LV
            MeuLV.Label2.Caption = FiltroGeral
            ...
            MeuLV.Visible = True
            Exit Sub
        'CONSULTA PREVISÃO FINANCEIRA - RECEITA
        ElseIf QualLV = 3 Then
            ...
            Formulario = "Receitas"
            LegendaExc = "Receita" 'Usado na mensagem de exclusão
            indiceVarGlobal = 1
            If Pesquisa <> "excluir" And Pesquisa <> "novo" And Pesquisa <> "editar" And Pesquisa <> "0" Then
                MontaFiltro
                If FiltroGeral = "" Then
                ...
                End If
                carregaTABS "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""
                If FiltroGeral = "Todos" Then SqlLV = "select CONVERT(VARCHAR,A.DATAVENCIMENTO,103) DATAVENCIMENTO,A.CODNATFIN,A.DESC_NATUREZA,A.PREVISTO_INICIAL,A.VLR_PREVISTO,A.VALOR_REALIZADO,CASE WHEN A.VLR_PREVISTO = 0 THEN A.PREVISTO_INICIAL-A.VALOR_REALIZADO+A.AJUSTE ELSE A.VLR_PREVISTO-VALOR_REALIZADO+A.AJUSTE END AS SALDO_PREVISTO,CASE WHEN A.VLR_PREVISTO>0 THEN A.PREVISTO_INICIAL-VLR_PREVISTO ELSE 0 END EMPRESTIMO,A.AJUSTE, 0 as ACUMULADO from " & _
                                                      "(select DATAVENCIMENTO AS DATAVENCIMENTO,CODNATFIN AS CODNATFIN,(SELECT FTB1.DESCRICAO FROM " & vBancoSAP & ".DBO.FTB1 WHERE CODTB1FLX collate SQL_Latin1_General_CP1_CI_AI = CODNATFIN AND FTB1.CODCOLIGADA=1) AS DESC_NATUREZA,isnull((SELECT (VALORPREVISTO) FROM " & vBancoSAP & ".DBO.##CALC_PREVISTO where CODNATFIN=R.CODNATFIN AND R.DATAVENCIMENTO=##CALC_PREVISTO.DATAVENCIMENTO),0) AS PREVISTO_INICIAL, " & _
                                                      "ISNULL(VLR_UTILIZADO,0) + isnull((SELECT (VALORPREVISTO-VALORUTILIZADO) FROM " & vBancoSAP & ".DBO.##CALC_PREVISTO where CODNATFIN=R.CODNATFIN AND R.DATAVENCIMENTO=##CALC_PREVISTO.DATAVENCIMENTO),0) AS VLR_PREVISTO,VALOR_REALIZADO AS VALOR_REALIZADO, 0 AJUSTE from " & vBancoSAP & ".DBO.##PREVISTO_REALIZADO R " & _
                                                      "UNION ALL " & _
                                                      "SELECT DATAVENCIMENTO AS DATAVENCIMENTO,CODNATFIN AS CODNATFIN,(SELECT FTB1.DESCRICAO FROM " & vBancoSAP & ".DBO.FTB1 WHERE CODTB1FLX collate SQL_Latin1_General_CP1_CI_AI = CODNATFIN AND FTB1.CODCOLIGADA=1) AS DESC_NATUREZA,VALORPREVISTO AS PREVISTO_INICIAL,VALORPREVISTO-VALORUTILIZADO,0 AS VALOR_REALIZADO,0 AJUSTE " & _
                                                      "FROM " & vBancoSAP & ".DBO.##CALC_PREVISTO where DATAVENCIMENTO NOT IN(SELECT DATAVENCIMENTO FROM " & vBancoSAP & ".DBO.##PREVISTO_REALIZADO WHERE DATAVENCIMENTO <> ##CALC_PREVISTO.DATAVENCIMENTO)) as A order by 1"
            Else
                If MeuLV.Visible = True Then Unload MeuLV
            End If
            ...
            MeuLV.cmdconsulta(0).Visible = False
            QtdColReal = 0
            MontaCabLV "Vencimento", "Nat. Finan", "Descrição", "Prev. Inicial", "V. Previsto", "V. Realizado", "Saldo Prev", "Emprestimo", "Ajuste", "Acumulado", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""
            DimensionaLV "Receitas"
            MontaCabecalhoLV
            MontaDadosLV "D"
            If checaFiltro = True Then
                'PersonaColLV 11, "N", "N", "", "N", "N", "S", "D"
                ...
                PersonaColLV 7, "N", "N", "", "N", "N", "S", "D"
                PersonaColLV 8, "N", "N", "", "N", "N", "S", "D"
                PersonaColLV 9, "S", "S", "", "N", "N", "S", "D"
                'PersonaColLV 16, "N", "N", "", "S", "N", "N", "E"
            End If
            If MeuLV.ListView1.ListItems.Count > 0 Then ajusta_LV
            MeuLV.Label2.Caption = FiltroGeral
