...
Private Sub Form_Load()
    ...
    If apontaLV = 16 Then
        SqlAvaHab = "Select c.cpf,c.codcolaborador,'INTD ' + c.nomecolaborador as nomecolaborador,c.foto,d.nomehabilidade,d.descricao,a.pontuacao,f.nomecargo,g.logo,i.mediaaprovacao,i.aprovadorest,j.nomedepartamento,k.nomesetor from " & _
                   "tbINTDHab as a inner join   tbINTD as b on a.codINTD = b.codINTD inner join tbColaboradores as c on b.codcolaborador = c.id inner join tbhabilidades as d on a.codHabilidade = d.codhabilidade " & _
                   "inner join tbmatriz as e on b.codmatriz = e.codmatriz inner join tbcargos as f on e.codcargo = f.codcargo inner join tbdepartamentos as j on e.coddepartamento = j.coddepartamento " & _
                   "inner join tbsetores as k on e.codsetor = k.codsetor inner join tbDadosEmpresa as g on g.codcoligada = '" & vCodcoligada & "', tbParametros as i where a.codcoligada = '" & vCodcoligada & "' and a.codINTD = '" & Val(varGlobal) & "'"
    Else
