...
Private Sub cmdCadastro_Click(Index As Integer)
    ...
    Case 4
        IncluirSetor 4
    Case 5
        If MsgBox("Deseja EXCLUIR esse setor da guia introdutório?", vbQuestion + vbYesNo, "SGCH") = vbYes Then
    ...
    Case 7
        IncluirSetor 7
    Case 8
        If MsgBox("Deseja EXCLUIR esse nível?", vbQuestion + vbYesNo, "SGCH") = vbYes Then
...
Private Sub listview_cabecalho()
    ...
    ListView5.ColumnHeaders.Add , , "Código", ListView4.Width / 12
...
'On Error GoTo TrataErro
    ...
    If lblStatusRev = "REVISADO" And Check4.Value = 1 Then
        If MsgBox("Foram realizadas novas revisões. Deseja realizar novas programações?", vbQuestion + vbYesNo, "SGCH") = vbYes Then
            '>>>>>>>>>>> ROTINA EM TESTE <<<<<<<<<<<<<<<<<<<<<<<
            ...
            '<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
            Else
                'QUERY DE TREINAMENTOS INTRODUTORIOS/OBRIGATORIOS PARA COLABORADORES QUE ESTÃO EM SEUS CARGOS
                ...
                rsListaIntObrINTD.Open SqlListaIntObrINTD, cnBanco, adOpenKeyset, adLockReadOnly
                'Se a query acima não retornar ZERO, realiza os procedimentos abaixo
                If rsListaIntObrINTD.RecordCount > 0 Then
                    'Se o trienamento estiver marcado como introdutório
                    If Check1.Value = 1 Then
                        ListView2.ListItems.Item(1).Selected = True
                        If ListView2.ListItems.Item(1) = "000" Then
                            While Not rsListaIntObrINTD.EOF
                                vINTD = rsListaIntObrINTD.Fields(3)
                                'Abaixo Grava o treinamento introdutorio para todos os colaboradores que estão em INTDs abertas
                        ...
                        End If
                    End If
                    'Se o trienamento estiver marcado como obrigatório
                    If Check2.Value = 1 Then
                        ...
                        If ListView3.ListItems.Item(1) = "000" Then
                            While Not rsListaIntObrINTD.EOF
                                vINTD = rsListaIntObrINTD.Fields(3)
                                'Abaixo Grava o treinamento obrigatorios para todos os colaboradores que estão em INTDs abertas
                                GravaTreiIntObr rsListaIntObrINTD(0), rsListaIntObrINTD(1), txtCadTreinamento(0)
                                'Abaixo Grava treinamento para ser visualizado na INTD
                                If vINTD <> 0 Then gravaDadosINTDCur
                                rsListaIntObrINTD.MoveNext
                ...
                rsListaIntObr.Close
                Set rsListaIntObr = Nothing
                MsgBox "A revisão deste curso/treinamento gerou novas programações a serem agendadas", vbInformation, "SGCH"
            End If
        End If
    End If
    '*********************************************
...
Private Function ValidaCampo()
    ...
    If txtCadTreinamento(0).Text = "" Then
        MsgBox "Favor informar o campo " & Me.txtCadTreinamento(0).Tag, vbInformation, "Atenção"
        Me.txtCadTreinamento(0).SetFocus
        Exit Function
    End If
    If txtCadTreinamento(1).Text = "" Then
        MsgBox "Favor informar o campo " & Me.txtCadTreinamento(1).Tag, vbInformation, "Atenção"
        Me.txtCadTreinamento(1).SetFocus
        Exit Function
    End If
    If txtCadTreinamento(2).Text = "" Then
        MsgBox "Favor informar o campo " & Me.txtCadTreinamento(2).Tag, vbInformation, "Atenção"
        Me.txtCadTreinamento(2).SetFocus
        Exit Function
    End If
    If txtCadTreinamento(4).Text = "" Then
        MsgBox "Favor informar o campo " & Me.txtCadTreinamento(4).Tag, vbInformation, "Atenção"
...
Private Sub ResultPesq()
    ...
    If rsTreinamentos.RecordCount > 0 Then
        ...
        FecharAgrup
    Else
        MsgBox "Treinamento não encontrado"
    End If
    rsTreinamentos.Close
    Set rsTreinamentos = Nothing
End Sub
