...
Private Sub CompoeProgs()
    ...
    sqlCompoeProgs = "select a.codprogramacao,b.nometreinamento,c.datainicio,c.datafim from tbpendentescur as a inner join tbtreinamentos as b on a.codcoligada = '" & vCodcoligada & "' and a.codtreinamento = b.codtreinamento inner join tbprogramacao as c on a.codprogramacao = c.codprogramacao inner join tbColaboradores as d on a.cpf = d.cpf " & _
                     "where a.status = 'Agendado' or a.status = 'Reagendado' and d.ativo= 'S' group by a.codprogramacao,b.nometreinamento,c.datainicio,c.datafim order by a.codprogramacao"
