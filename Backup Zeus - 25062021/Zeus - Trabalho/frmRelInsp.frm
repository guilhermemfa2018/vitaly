...
'    sqlLisview = "Select a.fce,max(d.codprojeto) as codprojeto,d.projeto,MAX(a.codlm) as codlm,Max(a.codseq) as codseq,c.desenho,Max(c.revisao) as revisao,b.descposicao as descricao,b.posicao as posicao,MAX(a.quantcj) as quantidade,MAX(b.pesoposicao) AS PesoPosicao,e.idoperacao,e.idcc,MAX(e.status) as status,case when sum(f.qtdlib) is null then 0 else sum(f.qtdlib) end as qtdlib,case when MAX(a.quantcj)- sum(f.qtdlib) is null then MAX(a.quantcj) else MAX(a.quantcj)- sum(f.qtdlib) end as qtddisp, " & _
'                 "MAX(a.area) as pintura from tbItemLM as a inner join tbPosicoes as b on a.codigopos = b.codigopos inner join tbDesenhos as c on a.codigodes = c.iddesenho inner join tbProjetos as d on c.codprojeto = d.codprojeto inner join tbositens as e on a.fce = e.fce and a.codlm = e.codlm and a.codseq = e.codseq left join tbrelinspexpitens as f on a.fce = f.fce " & _
'                 "and d.codprojeto = f.codprojeto and a.codlm = f.codlm and a.codseq = f.codseq where a.fce = '" & Val(Mid(varGlobal, 7, 4)) & "' and d.codprojeto = '" & Val(Mid(varGlobal, 1, 6)) & "' and e.idcc = '7000.7103.SC-02' and e.status = 3 or a.fce = '" & Val(Mid(varGlobal, 7, 4)) & "' and d.codprojeto = '" & Val(Mid(varGlobal, 1, 6)) & "' and " & _
'                 "e.idcc = '7000.7103.SC-03' and e.status = 3 group by a.fce,d.projeto,c.desenho,b.posicao,b.descposicao,e.idoperacao,e.idcc order by a.fce,d.projeto,c.desenho,b.posicao,e.idoperacao"

'    sqlLisview = "select T1.fce,max(T1.codprojeto) as codprojeto,T1.projeto,max(T1.codlm) codlm,max(T1.codseq) codseq,T1.desenho,max(T1.revisao) as revisao,T1.descricao,T1.posicao,max(T1.quantidade) as quantidade,max(T1.PesoPosicao) as PesoPosicao,T2.idoperacao,T2.idcc,max(T2.status) as status,case when sum(T1.qtdlib) is null then 0 else sum(T1.qtdlib) end as qtdlib,case when MAX(T1.quantcj)- sum(T1.qtdlib) is null then MAX(T1.quantcj) else MAX(T1.quantcj)- sum(T1.qtdlib) end as qtddisp,max(T1.pintura) as pintura," & _
'                 "T2.IDOS from (Select a.fce,d.codprojeto,d.projeto,a.codlm,a.codseq,c.desenho,c.revisao,b.descposicao as descricao,b.posicao as posicao,a.quantcj as quantidade,b.pesoposicao,f.qtdlib,a.quantcj,a.area as pintura From tbItemLM as a inner join tbPosicoes as b on a.codigopos = b.codigopos inner join tbDesenhos as c on a.codigodes = c.iddesenho inner join tbProjetos as d on c.codprojeto = d.codprojeto left join tbrelinspexpitens as f on a.fce = f.fce and d.codprojeto = f.codprojeto and a.codlm = f.codlm and " & _
'                 "a.codseq = f.codseq) T1 inner join (Select a.fce,a.codlm,a.codseq,e.idoperacao,e.idcc,e.status,e.idos from tbItemLM as a inner join tbositens as e on a.fce = e.fce and a.codlm = e.codlm and a.codseq = e.codseq AND E.fce = '" & Val(Mid(varGlobal, 7, 4)) & "') T2 on T1.fce = T2.fce and T1.codlm = T2.codlm and T1.codseq = T2.codseq where T1.fce = '" & Val(Mid(varGlobal, 7, 4)) & "' and T1.codprojeto = '" & Val(Mid(varGlobal, 1, 6)) & "' and T2.idcc = '7000.7103.SC-02' and T2.status >= 2 or T1.fce = '" & Val(Mid(varGlobal, 7, 4)) & "' and T1.codprojeto = '" & Val(Mid(varGlobal, 1, 6)) & "' and T2.idcc = '7000.7103.SC-03' and T2.status >= 2 group by T1.fce,T1.projeto," & _
'                 "T1.desenho,T1.posicao,T1.descricao,T2.idoperacao,T2.idcc,t2.idos order by T1.fce,T1.projeto,T1.desenho,T1.posicao,T2.idoperacao"
