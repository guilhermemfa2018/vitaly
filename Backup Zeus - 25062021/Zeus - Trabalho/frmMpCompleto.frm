...
Begin VB.Form frmMPCompleto 
   ...
   Begin VB.Frame Frame21 
      ...
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel5 
         Height          =   255
         Left            =   9960
         OleObjectBlob   =   "frmMpCompleto.frx":BF74
         TabIndex        =   82
         Top             =   240
         Width           =   1455
      End
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel4 
         Height          =   255
         Left            =   4920
         OleObjectBlob   =   "frmMpCompleto.frx":BFE4
         TabIndex        =   81
         Top             =   240
         Width           =   855
      End
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel3 
         Height          =   255
         Left            =   3240
         OleObjectBlob   =   "frmMpCompleto.frx":C04C
         TabIndex        =   80
         Top             =   240
         Width           =   495
      End
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel2 
         Height          =   255
         Left            =   1680
         OleObjectBlob   =   "frmMpCompleto.frx":C0AC
         TabIndex        =   79
         Top             =   240
         Width           =   1455
      End
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel1 
         Height          =   255
         Left            =   120
         OleObjectBlob   =   "frmMpCompleto.frx":C122
         TabIndex        =   78
         Top             =   240
         Width           =   1455
      End
...
'On Error GoTo Err
    ...
    If txtformula(17).Text <> "" Then
        ...
        sqlOSstatus = "select * from tbos as a where a.idos = '" & Val(txtformula(17).Text) & "' and revisao = '" & Val(txtformula(18).Text) & "'"
        ...
        vQualquerDado(6, 2) = "I"
        GravaDados "tbOS", "idos", "I", txtformula(17), 6, "", "", txtformula(17)
...
'        LimpaControles
        txtformula(11) = ""
'        txtformula(11) = Format(GeraCodigoTB("tbMP", "idprogramacao"), "000000")
    ElseIf Status = "editar" Then
        ResultPesq
...
'    configControles
    ...
    'Dim rsDeletar As New ADODB.Recordset
    'Dim sqlDeletar As String
    'sqlDeletar = "Delete from tbMPDesSel" & txtformula(11).Text
    'rsDeletar.Open sqlDeletar, cnBanco
...
Private Sub mostraDesenhos(vTabela As String, TV As TreeView)
    ...
    ElseIf vTabela = "tbMPDesSel" & vTime Then
        ...
        '"left join CORPORERM.dbo.TTB2 as e on b.CODTB2FAT = e.CODTB2FAT inner join tbMPDesSel as f on a.fce = f.fce and a.codlm = f.codlm and a.codseq = f.codseq inner join tbProjetos as g on g.codprojeto = c.codprojeto left join tbositens as h on a.fce = h.fce and a.codlm = h.codlm and a.codseq = h.codseq Where a.fce = '" & Val(txtformula(12)) & "' and g.projeto = '" & txtformula(13) & "'" & _
        '"Group by c.desenho,c.revisao,b.CODIGOPRD + ' - ' + cast(a.codmat as varchar),b.NOMEFANTASIA,d.posicao,d.item,a.quantcj,a.quantunit,a.dimensoes,a.pesounit,d.descposicao,a.codlm,a.codseq Order by c.desenho,d.posicao,b.NOMEFANTASIA"
...
' ORIGINAL    vJuntaNome = rsTreeview.Fields(0) & " (" & rsTreeview.Fields(1) & ");" & rsTreeview.Fields(10) & " - " & rsTreeview.Fields(4) & ";" & rsTreeview.Fields(5) & " - " & rsTreeview.Fields(3) & " (" & rsTreeview.Fields(8) & ") - ID: " & Format(rsTreeview.Fields(11), "00") & Format(rsTreeview.Fields(12), "000")
    ...
    Do While Not rsTreeview.EOF
        ...
        'Teste
'        If Mid$(right(vNome1, 14), 1, 3) = "OS:" Then
'            Dim vTamanho As Integer
'            vTamanho = Len(vNome1) - 11
'            vNome1 = Mid$(vNome1, 1, vTamanho) & ")"
'        End If
        
        
        
        Do While Mid$(vNome1, 1, Len(vNome1) - 1) = Mid$(vNomeA, 1, Len(vNome1) - 1) And Not rsTreeview.EOF
'       Do While vNome1 = Mid$(vNomeA, 1, Len(vNome1)) And Not rsTreeview.EOF
            If vNomeB <> "" Then
                vNo = vNo + 1
                vNomeNo = vNome2 & vNo
                'SEGUNDO NO
...
'        SqlBuscaPeso = "select a.quantunit*a.pesounit as PesoTotal from tbItemLM as a where a.fce = '" & Val(txtformula(12)) & "' and a.codlm = '" & Val(vCodLM) & "' and a.codseq = '" & Val(vCodSeq) & "'"
...
Private Sub compoeControlesOS()
    ...
    SqlCompoeOS = "select * from tbos where idos = '" & Val(vPonte1) & "'"
...
Private Sub clonarOS()
    ...
    LimpaLV ListView1
'    chamaSQL "select a.idsequencia,RIGHT('000000000'+ CONVERT(VARCHAR,a.idos),9) + '/' + a.revisaoos,a.idcc,a.nomecc,a.desenhos,'',a.tempocalc,a.grupo,a.idprogramacao,a.variaveis,a.observacao,a.idoperacao,a.codigobarra,a.status,Replicate ('0',9 - Len(Cast(a.idos as varchar))) + Cast(a.idos as varchar) + Replicate ('0',3 - Len(Cast(a.idoperacao as varchar))) + Cast(a.idoperacao as varchar)  as ordenação  from tbMPItens as a where a.idprogramacao = '" & Val(vClone) & "'"
...
Private Function GravaLV1()
    ...
    Dim Y As Integer, X As Integer
    Dim vGuardaData(49, 3) As String
