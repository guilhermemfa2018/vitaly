...
'                If FiltroGeral = "Não ativos" Then SqlLV = "select a.codmaterial,a.descricao,a.unidade,a.formula,a.forpint,a.constpint,a.ativo from tbMateriais as a where a.ativo <> 'S'"
        ...
        ElseIf QualLV = 5 Then
            ...
            MeuLV.cmdconsulta(12).Visible = False
            QtdColReal = 0
            MontaCabLV "FO", "Empresa", "Coleta nº", "Contato", "Fone", "Descrição", "Data Abertura", "Dev. CP", "Proposta nº", "Quant.", "Valor Unit", "Valor Total", "Pedido nº", "FCE nº", "Status FO", "Ativo", "Status FCE", "", "", "", ""
...
'        'FCE - Ficha de Controle de Encomenda
        ElseIf QualLV = 6 Then
            ...
            MeuLV.cmdconsulta(12).Visible = False
            QtdColReal = 0
            MontaCabLV "Data abertura", "FCE", "Cliente", "Contato", "Fone", "Data entrega", "Pintura", "Transporte", "Matéria-prima", "Fabricação", "Reparo", "Ativo", "Status FCE", "", "", "", "", "", "", "", ""
...
'ESTA OCORRENDO PROBLEMAS DEVIDO A MENSAGEM DE GRAVAÇÃO AO SAIR DO FORMULARIO 'FRMLM'
                ...
                If FiltroGeral = "Todos" Then SqlLV = "select right('0000' + rtrim(a.fce),4),a.codlm,a.dataabertura,a.descricao,a.ativo,CASE WHEN B.status = 0 THEN 'ANDAMENTO' WHEN B.status = 1 THEN 'CONCLUIDA' WHEN B.status = 2 THEN 'PARALIZADA' END AS STATUS from tblm as a inner join tbFCE as b on a.fce = b.fce order by a.fce, a.codlm"
                If FiltroGeral = "Ativos" Then SqlLV = "select right('0000' + rtrim(a.fce),4),a.codlm,a.dataabertura,a.descricao,a.ativo,CASE WHEN B.status = 0 THEN 'ANDAMENTO' WHEN B.status = 1 THEN 'CONCLUIDA' WHEN B.status = 2 THEN 'PARALIZADA' END AS STATUS from tblm as a inner join tbFCE as b on a.fce = b.fce where a.ativo = 'S' order by a.fce, a.codlm"
                If FiltroGeral = "Não ativos" Then SqlLV = "select right('0000' + rtrim(a.fce),4),a.codlm,a.dataabertura,a.descricao,a.ativo,CASE WHEN B.status = 0 THEN 'ANDAMENTO' WHEN B.status = 1 THEN 'CONCLUIDA' WHEN B.status = 2 THEN 'PARALIZADA' END AS STATUS from tblm as a inner join tbFCE as b on a.fce = b.fce where a.ativo <> 'S' order by a.fce, a.codlm"
            Else
                If MeuLV.Visible = True Then Unload MeuLV
            End If
            ...
            MeuLV.cmdconsulta(12).Visible = False
            QtdColReal = 0
            MontaCabLV "FCE", "LM", "Data Abertura", "Descrição", "Ativo", "Status FCE", "", "", "", "", "", "", "", "", "", "", "", "", "", "", ""
...
'                                                          "inner join tbFCE as h on b.fce = h.fce where a.status = 3 group by a.idprogramacao,c.idos,f.revisao,a.dataprogramacao,b.fce,b.projeto,a.responsavel,a.ativo,a.status,H.STATUS order by a.idprogramacao"
            ...
            QtdColReal = 0
            MontaCabLV "Planejamento", "OS nº", "Rev.", "Data", "FCE", "Projeto", "Responsável", "Desenho", "Ativo", "Retrabalho", "Status", "Status FCE", "", "", "", "", "", "", "", "", ""
...
'                PersonaColLV 6, "N", "N", "", "S", "N", "N", "E"
        ...
        ElseIf QualLV = 20 Then
            ...
            If Pesquisa <> "excluir" And Pesquisa <> "novo" And Pesquisa <> "editar" And Pesquisa <> "0" Then
                ...
                If FiltroGeral = "" Then frmFiltro.Show 1
                If MeuLV.Visible = True Then Unload MeuLV
'NÃO FUNCIONA COM TABELAS TOTVS
                carregaTABS "", "", "", "", "", "", "", "", "", ""

'                If FiltroGeral = "Todos" Then SqlLV = "SELECT T1.DESCRICAO,T1.CODTB3FAT,T1.PESO_LIQUIDO,T1.PESO_BRUTO,T1.VALOR_BRUTO,T1.VALOR_LIQUIDO,T1.DTCRIACAO,T2.VALOR_ORIGINAL,T2.VALOR_BAIXADO,T2.VALOR_RECEBER,T3.PESO,T3.VALOR_TOTAL FROM " & _
'                                                      "    (select  " & _
...
'                                                      "    LEFT JOIN CORPORERM.dbo.TMOV AS B ON A.CODTB3FAT = B.CODTB3FAT AND B.CODTMV in ('2.2.01','2.205','1.2.15','1.2.17') AND B.STATUS <> 'C' " & _
