...
Attribute VB_Ext_KEY = "PropPageWizardRun" ,"Yes"
 ...
 ' *******************************
 ' ** THIS CODE IS BETA AND RAW **
 ' ** IT HAS NOT BEEN CLEANED UP *
 ' ** OR ENHANCED. IT DOES WORK **
 ' ** THOUGH WITH NO APPARENT ****
 ' ** BUGS OR RESOURCE LEAKS *****
 
 ' I have been sitting on this code for more than 6 months
 ' promising I'd find time to come back and
 ' clean the source code.
 ' I give up...you clean it up!
 ...
 ' LocalColorTables
 Dim arrayLocalColorTables() As ColorTable
 ...
 ' Flag to signal we should try and automatch
 ' the backcolor of the control to the Background
 ' of the Animated Gif.
 Dim mAutoBackColor As Boolean
...
Attribute StopRender.VB_ProcData.VB_Invoke_Property = "PropPage1"
 ...
 Property Get AutoBackColor() As Boolean
 ' Flag to tell us to auto match Background color
 ' of Control to the Animated Gif.
 ' We do this by examining the Top Leftmost
 ' pixel in the first Frame of the Gif
 AutoBackColor = mAutoBackColor
 End Property
 
 Property Let AutoBackColor(blS As Boolean)
 mAutoBackColor = blS
 End Property
 ...
 ' SOme older Gif's actaully have LogWidth and LogHeight
 ' values smaller than the ImageWidth and ImageHeight values
 ' of the individual Gif Frames.
 ' Let's check and adjust accordingly.
 For x = 0 To UBound(ImageWidth)
 If ImageWidth(0) > LogWidth Then LogWidth = ImageWidth(x)
 If ImageHeight(0) > LogHeight Then LogHeight = ImageHeight(x)
 Next x
 ...
 ' The last instance is deleted when Class exits
 ...
 rc.Right = bmap.bmWidth
    ...
    ' Does user want us to autoselect the COntrol's BackColor
    ' to match the current Gif?
    ' We do this by examining the Top Leftmost
    ' pixel in the first Frame of the Gif or by
    ' using the BackGroundColorIndex.
    ' Neither logic works 100% of the time. It depends
    ' on the software used to create the Gif and the
    ' user who designed it. Unfortunately there is no
    ' exact standard that everyone followed 100%!
    ' Just uncomment the code of your choice to
    ' enable the AutoBackColor functionality.
    If AutoBackColor Then
    ' Can't do this because GetPixel doesn't work on MEMORY DC's!!!!
    ' Let's use BackColor Index and
        If Not TransparentColorFlag(0) Then
          
    'apiCopyMemory bTemparray(0), arrayLocalColorTables(ctr).ct(BackgroundColorIndex * 3), 3
    'BackColor = GetPixel(hDCbuffer, 1, 1)
    'BackColor = RGB(bTemparray(0), bTemparray(1), bTemparray(2))
  
    ' Let's copy one pixel to use GetPixel against
    'lngRet = apiBitBlt(hdc, 0, 0, 4, 4, hDCbuffer, 0, 0, SRCCOPY)
    'BackColor = GetPixel(hdc, 1, 1)
 ...
 ' Thanks to Thomas for finding this resource leak!
 lngRet = apiDeleteObject(hBmpOrig)
 ...
 End Function
    ...
    ' Offset to this Byte
    Dim ImageDescPackedFieldsByte As Long
    ...
    'lc = 0
     ...
     If GlobalColorTableFlag Then
         ...
         'apiCopyMemory arrayLocalColorTables(lc).ct(0), bArray(lngCurPosition + 1), (3 * (2 ^ (SizeOfGlobalColorTable + 1)))
    ...
    ReDim Preserve lngStartLocalColorTable(lc)
    
   ReDim Preserve arrayLocalColorTables(lc)
    
    If GlobalColorTableFlag Then
    apiCopyMemory arrayLocalColorTables(lc).ct(0), GlobalColorTable(0), (3 * (2 ^ (SizeOfGlobalColorTable + 1)))
       End If
...
' No harm done becasue I store the derived vars from Packed fields individually.
  ...
  ' Just moved here May 25. Really should check
  ' to see if we are past the end of the file
  ' before we add 1 to the loop counter.
  ' Check to make sure we are not at the end of the file
  If bArray(lngCurPosition) = Trailer Then 'Exit Do
  If lngCurPosition >= FLength - 4 Then Exit Do
  End If
  lngRet = 0
...
Loop
  ...
  If bArray(lngCurPosition) = Trailer Then 'Exit Do
    ...
    ' Need this offset so we can jump over the
    ' LocalColorTable when we write out data to file
    'ImageDescPackedFieldsByte = lngCurPosition + 9
    ...
    If LocalColorTableFlag(lc) Then
        ...
        ' THe LocalColorTable array is still used in the
        ' Verify/Change duplicate Transparent Color Function
        ' SHould be ipdated to use arrayLocalColorTables(x).ct(x)
        apiCopyMemory LocalColorTable(0), bArray(lngCurPosition + 10), (3 * (2 ^ (SizeOfLocalColorTable(lc) + 1)))
        
        apiCopyMemory arrayLocalColorTables(lc).ct(0), bArray(lngCurPosition + 10), (3 * (2 ^ (SizeOfLocalColorTable(lc) + 1)))
        ...
        ' redo the code for the next version!
         ...
         ' Save offset
        lngStartLocalColorTable(lc) = lngCurPosition + 10
        
        
        ' Let's just fake it and simply convert a LocalColorTable to
        'a GobalColorTable...Doh!
       ' apiCopyMemory GlobalColorTable(0), bArray(lngCurPosition + 10), (3 * (2 ^ (SizeOfLocalColorTable(lc) + 1)))
        'LocalColorTableFlag(lc) = False
...
End If
    ...
    ' ********FIX AND BELOW AS WELL
   ' lngNewTransparent = 14
    ...
    'arrayLocalColorTables(ctr1).ct(0)
    arrayLocalColorTables(lc).ct((3 * TransparentColorIndex(lc))) = (lngNewTransparent And &HFF)
    arrayLocalColorTables(lc).ct(((3 * TransparentColorIndex(lc)) + 1)) = (lngNewTransparent And &HFF00&) \ &H100
    arrayLocalColorTables(lc).ct(((3 * TransparentColorIndex(lc)) + 2)) = (lngNewTransparent And &HFF0000) \ &H10000
 
    
    
    ' Write to raw byte array
    bArray(lngStartLocalColorTable(lc) + (3 * TransparentColorIndex(lc))) = (lngNewTransparent And &HFF)
    bArray(lngStartLocalColorTable(lc) + (3 * TransparentColorIndex(lc)) + 1) = (lngNewTransparent And &HFF00&) \ &H100
    bArray(lngStartLocalColorTable(lc) + (3 * TransparentColorIndex(lc)) + 2) = (lngNewTransparent And &HFF0000) \ &H10000
 ...
 End If        ' For debugging
   ...
   ' lngRet = WriteFile(hFile, bArray(1), mGifStart(0) - 1, lngBytesWritten, 0)
    ' Above works REmm'd out May21 11:01Pm
    ' Adding support for Local ColorTables.
    ' We do this by adding a Full ColorTable(256 entries)
    ' After the Header for each Gif. If this Gif
    ' has a Local COlor Tabel then we simply copy
    ' it into these slots otherwise we leave along
    ' becasue there is obviously already a GLobal COlor Table
    
    
    
    ' If not LocalColorTabel then Skip
     If LocalColorTableFlag(ctr1) Then
        
        ' Set LogWidth and Height from Image
        
        'If mGifStart(0) - 1 < 768 + 13 Then
        ' Set GlobalColorTableFlag to 1
        ' Set Size Of Global Color table to 7
        ' Remember, The Gif spec has us add 1 to the Size of Global
        ' Color Table value before processing.
        ' So we for this PackedBits field we want 10000111 = 135
        ' We find the Logical Screen Descriptor Packed Fields
        ' property at Byte 11 from the start of our file.
        bTemp = SizeOfLocalColorTable(ctr1)
        'bTemp = bTemp Or Bit8
        
        bArray(11) = bTemp Or Bit8
        ' Copy this Frames Local Color table into our
        ' GLobal Color Table
         'apiCopyMemory ,LocalColorTable(0), 256*3
        
    
        ' Finally write out our new Header and Global Color Table
        lngRet = WriteFile(hFile, bArray(1), 13, lngBytesWritten, 0)
    lngRet = WriteFile(hFile, arrayLocalColorTables(ctr1).ct(0), (3 * (2 ^ (SizeOfLocalColorTable(ctr1) + 1))), lngBytesWritten, 0)
    
    ' Set Local ColorTable flag to No
    ' Set Size of Local COlor Table to 0
    ' Do this by masking out unwanted bits
    ' ANd with 01111000 = 120
    bTemp = bArray(lngStartLocalColorTable(ctr1) - 1)
    bTemp = bTemp And 120
    bArray(lngStartLocalColorTable(ctr1) - 1) = bTemp
    
    'Write data for this Frame up to the start of the Local Color Table
    lngRet = WriteFile(hFile, bArray(mGifStart(ctr1)), (lngStartLocalColorTable(ctr1) - mGifStart(ctr1)) + 0, lngBytesWritten, 0)
   
   ' Write data  after the end of the Local Color Table
   lngTemp = mGifStart(ctr1) + 1 + lngBytesWritten + _
   (3 * (2 ^ (SizeOfLocalColorTable(ctr1) + 1)))
   'lngRet = WriteFile(hFile, bArray(lngTemp), _
   '(mGifEnd(ctr1) - mGifStart(ctr1)) - lngTemp + 14, lngBytesWritten, 0)
   
   ' Becuse we need the first byte of the ImageData Control Block
   ' we will backup one byte from the ImageDataBlock
   lngRet = WriteFile(hFile, bArray(lngTemp - 1), _
   (mGifEnd(ctr1) - lngTemp) + 1, lngBytesWritten, 0)
...
err_NoGif:
  ...
  End Function
    ...
    '  ********** CHANGE THIS SO WE DON"T NEED IT!!!!!!!!
   ' Fix trouble with LocalColorTables and
   ' specifically with the StartBurst Angel Gif
   apiCopyMemory GlobalColorTable(0), arrayLocalColorTables(ctr).ct(0), 768
    ...
    If TransparentColorFlag(ctr) Then
        ...
        'apiCopyMemory bTemparray(0), arrayLocalColorTables(ctr).ct(TransparentColorIndex(ctr) * 3), 3
        'apiCopyMemory bTemparray(0), GlobalColorTable((TransparentColorIndex(ctr) * 3)), 3
       
        'Else
        ' **BELOW WORKS! MAY 25 just changed
        apiCopyMemory bTemparray(0), GlobalColorTable((TransparentColorIndex(ctr) * 3)), 3
        'Debug.Print bTemparray(0), bTemparray(1), bTemparray(2) & "  TransIndex:" & TransparentColorIndex(ctr)
        'End If
    Else
        ...
        ' ************ BUG
        ' THis is not correct because GLobalColorTableFlag
        ' is always TRUE!!!!!!!!!!!
        
        ' ** JUST ADDED MAY 21  FOR STARBURST GIF PROBLEM
        'apiCopyMemory bTemparray(0), LocalColorTable(BackgroundColorIndex * 3), 3
        'apiCopyMemory bTemparray(0), arrayLocalColorTables(ctr).ct(BackgroundColorIndex * 3), 3
   ...
   ' no transparent background
    ...
    ' Vars were not being cleared in between Gif Loads
    Erase hStdPicture()
    Erase mGifStart()
    Erase mGifEnd()
    Erase GlobalColorTable()
    Erase LocalColorTable
    Erase bArray()
    ctr1 = 0
    NumberofFrames = 0
    ' If GlobalColorFlag is not cleared you
    ' will generate a RunTime error if you play
    ' a Global followed by a local ColorTable Gif
   ...
   ' temp var for DC's current BackColor
    ...
    ' Static blRendering As Boolean
    'Debug.Print "CurrentFrame:" & CurrentFrame
    'Debug.Print "Delay:" & (DelayTime(CurrentFrame) * 10)
    
    'If blRendering = True Then Exit Sub
    'blRendering = True
    
    ' Draw the current frame
    PaintGif CurrentFrame
    
    ' Boundary checking for damaged files
...
Attribute Refresh.VB_Description = "Forces a complete repaint of a object."
    ...
    ' Set Flag to try and AutoMatch Gif's Background
    ' Color for nonTransparent background Gif's
    AutoBackColor = True
