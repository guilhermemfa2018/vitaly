...
Begin VB.Form frmEmprestimo 
   ...
   Begin VB.Frame Frame3 
      ...
      Begin ACTIVESKINLibCtl.SkinLabel SkinLabel6 
         Height          =   255
         Left            =   2040
         OleObjectBlob   =   "adm.frx":12E0
         TabIndex        =   24
         Top             =   240
         Width           =   855
      End
   End
   Begin IMRM.chameleonButton cmdEmp 
      Height          =   615
      Index           =   1
      Left            =   1800
      TabIndex        =   12
...
Private vCodPrd As String

Private Sub cmdEmp_Click(Index As Integer)
    Select Case Index
    Case 0
        'Msgbox "Rotina em desenvolvimento", vbInformation, "IMRM"
        'Exit Sub
        
        If salvar_Dados = True Then
            mobjMsg.Abrir "Dados Salvos com sucesso!", Ok, informacao, "IMRM"
            varGlobal = txtEmprestimo(0).Text
            FCREmprestimo.Show 1
            Unload Me
        Else
            SkinLabel1.Visible = False
            mobjMsg.Abrir "Erro ao gravar dados", Ok, critico, "IMRM"
        End If
    Case 1
        Unload Me
    End Select
...
Private Sub cmdEmprestimo_Click(Index As Integer)
    ...
    Case 2
        ...
        SomaLV ListView2, 6, Text1
    Case 3
        If txtEmprestimo(0).Text = "" Or txtEmprestimo(0) = "" Then
            mobjMsg.Abrir "Informe um colaborador antes de realizar a pesquisa", Ok, critico, "Atenção"
        Else
            PesquisaProd
            ApagaExceso
        End If
...
Private Sub Command1_Click()
    If txtEmprestimo(0).Text = "" Or txtEmprestimo(1).Text = "" Then
        mobjMsg.Abrir "os campos CHAPA e NOME do colaborador devem estar preenchidos", Ok, informacao, "IMRM"
    Else
        frmDevolucao.Show 1
    End If
End Sub
...
'On Error GoTo ErrHandler
    ...
    If Status = "novo" Then
        ...
        txtEmprestimo(7).Text = Mid$(Principal.Caption, 43, 50)
    ...
    ElseIf Status = "editar" Then
        ...
        vCodLocalEstoque = vLocalEstoque
        ResultPesq
        chamaSQL "Select a.localestoque,a.codigoprd,a.qtdemprestado,a.um,a.descricao,'NÃO',a.valortotal,a.idprd from tbEmprestimoItens as a inner join tbEmprestimo as b on a.idmov = b.idmov and a.numeromov = b.numeromov where a.idmov = " & Mid$(varGlobal, 7, 6)
...
'    Me.ListView1.ColumnHeaders(7).Alignment = lvwColumnRight
    ...
    ListView4.ColumnHeaders.Add , , "MOV", ListView4.Width / 7
    ListView4.ColumnHeaders.Add , , "CHAPA", ListView4.Width / 8.5
    ListView4.ColumnHeaders.Add , , "FUNCIONARIO", ListView4.Width / 4
    ListView4.ColumnHeaders.Add , , "DATA", ListView4.Width / 5.5
    ListView4.ColumnHeaders.Add , , "USUARIO", ListView4.Width / 4
...
On Error GoTo Err
    ...
    If vNome = "" Then
        SqlchamaChapa = "select a.CODCOLIGADA,a.CODVEN,a.NOME,a.INATIVO,b.SITEMPRESTIMO,b.MOTBLOQUEIO,c.CODFUNCAO,d.NOME,c.CODSECAO,e.DESCRICAO from " & vBancoSAP & ".dbo.TVEN as a left join " & vBancoSAP & ".dbo.TVENCOMPL as b  on b.CODCOLIGADA=a.CODCOLIGADA and b.CODVEN=a.CODVEN left join " & vBancoSAP & ".dbo.PFUNC as c on a.CODVEN = c.CHAPA left join " & vBancoSAP & ".dbo.PFUNCAO as d on c.CODFUNCAO = d.CODIGO left join " & vBancoSAP & ".dbo.PSECAO as e on c.CODSECAO = e.CODIGO where a.CODCOLIGADA=1 and a.INATIVO=0 and a.codven = " & txtEmprestimo(0).Text & " and GETDATE() <= c.DATADEMISSAO or a.CODCOLIGADA=1 and a.INATIVO=0 and a.codven = " & txtEmprestimo(0).Text & " and c.DATADEMISSAO is null order by a.nome"
'        SqlchamaChapa = "select a.CODCOLIGADA,a.CODVEN,a.NOME,a.INATIVO,b.SITEMPRESTIMO,b.MOTBLOQUEIO,c.CODFUNCAO,d.NOME,c.CODSECAO,e.DESCRICAO from " & vBancoSAP & ".dbo.TVEN as a left join " & vBancoSAP & ".dbo.TVENCOMPL as b  on b.CODCOLIGADA=a.CODCOLIGADA and b.CODVEN=a.CODVEN left join " & vBancoSAP & ".dbo.PFUNC as c on a.CODVEN = c.CHAPA left join " & vBancoSAP & ".dbo.PFUNCAO as d on c.CODFUNCAO = d.CODIGO left join " & vBancoSAP & ".dbo.PSECAO as e on c.CODSECAO = e.CODIGO where a.CODCOLIGADA=1 and a.INATIVO=0 and a.codven = " & txtEmprestimo(0).Text & " AND C.CODSITUACAO in('A','F','P','Z') order by a.nome"
    Else
        SqlchamaChapa = "select a.CODCOLIGADA,a.CODVEN,a.NOME,a.INATIVO,b.SITEMPRESTIMO,b.MOTBLOQUEIO,c.CODFUNCAO,d.NOME,c.CODSECAO,e.DESCRICAO from " & vBancoSAP & ".dbo.TVEN as a left join " & vBancoSAP & ".dbo.TVENCOMPL as b  on b.CODCOLIGADA=a.CODCOLIGADA and b.CODVEN=a.CODVEN left join " & vBancoSAP & ".dbo.PFUNC as c on a.CODVEN = c.CHAPA left join " & vBancoSAP & ".dbo.PFUNCAO as d on c.CODFUNCAO = d.CODIGO left join " & vBancoSAP & ".dbo.PSECAO as e on c.CODSECAO = e.CODIGO where a.CODCOLIGADA=1 and a.INATIVO=0 and a.nome like '" & vNome & "%' AND GETDATE() <= c.DATADEMISSAO or a.CODCOLIGADA=1 and a.INATIVO=0 and a.nome like '" & vNome & "%' AND c.DATADEMISSAO is null order by a.nome"
...
On Error GoTo Err
    ...
    If vSqlp = "" Then
        Sqlp = "select a.CODCOLIGADA,a.CODVEN,a.NOME,a.INATIVO,b.SITEMPRESTIMO,b.MOTBLOQUEIO from " & vBancoSAP & ".dbo.TVEN as a left join " & vBancoSAP & ".dbo.TVENCOMPL as b  on b.CODCOLIGADA=a.CODCOLIGADA and b.CODVEN=a.CODVEN left join " & vBancoSAP & ".dbo.PFUNC as c on a.CODVEN = c.CHAPA where a.CODCOLIGADA=1 and a.INATIVO=0 AND GETDATE() <= c.DATADEMISSAO or a.CODCOLIGADA=1 and a.INATIVO=0 AND c.DATADEMISSAO is null"
    Else
        Sqlp = vSqlp
        vSqlp = ""
    End If
...
Private Sub MarcaDesmarca(LV As ListView)
    ...
    If LV.Name = "ListView1" Then
        vCodPrd = LV.SelectedItem.ListSubItems.Item(1)
    End If
    vPosicaoAtual = J
End Sub
...
'    vQualquerDado(11, 2) = "S"
    ...
    txtEmprestimo(10) = vSerie
    GravaDados "tbEmprestimo", "numeromov", "S", txtEmprestimo(9), 15, "serie", "S", txtEmprestimo(10)
...
Private Function GeraNumeroMov()
    ...
    'vLocalEstoque
    SqlGeraNumeroMov = "Select top 1 * from tbMov as a where a.serie = '" & SerieEmpresa & "E" & vLocalEstoque & "'  and codcoligada = 1 order by numeromov Desc"
    ...
    Else
        vNumeromov = 1
    End If
    vSerie = SerieEmpresa & "E" & vLocalEstoque
    ...
    vOK = False
    While vOK = False
        SqlVerTmov = "select * from tMov where Numeromov = '" & vNumeromov & "' and serie = '" & vSerie & "'"
        rsVerTmov.Open SqlVerTmov, cnBancoSAP, adOpenKeyset, adLockReadOnly
        If rsVerTmov.RecordCount > 0 Then
            vNumeromov = vNumeromov + 1
        Else
            vOK = True
        End If
        rsVerTmov.Close
    ...
    limpaQualquerDado
    vQualquerDado(1, 1) = vNumeromov ' VNUMEROMOV - Numero do movimento gerado pelo sistema deferramentaria
    vQualquerDado(1, 2) = "S"
...
Private Function GeraIDMov()
    ...
    vOK = False
    While vOK = False
        SqlVerTmov = "select * from tMov where idmov = " & vIDMov
        rsVerTmov.Open SqlVerTmov, cnBancoSAP, adOpenKeyset, adLockReadOnly
        If rsVerTmov.RecordCount > 0 Then
            vIDMov = vIDMov + 1
        Else
...
Private Sub ResultPesq()
    SqlCriterio = "Select a.chapa,a.nome,dataemprestimo,a.numeromov,a.nomequememprestou from tbEmprestimo as a where codcoligada = 1 and a.localestoque = " & Val(vCodLocalEstoque) & " and a.idmov = " & Mid$(varGlobal, 7, 6)
...
Err:
    If Err.Number = -2147217873 Then
        mobjMsg.Abrir "O IDMOV nº" & vIDMov & " Já existe na tabela TMOV", Ok, critico, "Ferramentaria"
        End
    End If
    Exit Sub
End Sub
...
Private Sub chamaHistorico()
    ListView4.ListItems.Clear
    chamaSQL "select a.serie as mov,b.chapa,b.nome as funcionario,CONVERT (VARCHAR, a.dataemprestimo, 103) as data,a.nomequememprestou " & _
             "from (select codigoprd,serie,dataemprestimo,nomequememprestou,idmov,descricao from tbEmprestimoItens where codcoligada = 1 union " & _
             "     select codigoprd,serie,datadevolucao,nomequememprestou,idmov,descricao from tbDevolucaoItens where codcoligada = 1) as a " & _
             "inner join (select chapa,nome,idmov from tbEmprestimo where codcoligada = 1 union " & _
             "           select chapa,nome,idmov from tbDevolucao where codcoligada=1) as b " & _
             "on a.idmov = b.idmov where a.codigoprd= '" & vCodPrd & "' order by a.descricao,b.chapa,a.dataemprestimo"
    Compoe_Listview ListView4, Sqlp, "0000"
End Sub
