...
Private Sub cmdCadastro_Click(Index As Integer)
    Select Case Index
    Case 0
        mobjMsg.Abrir "Deseja salvar os dados da Avaliação?", YesNo, pergunta, "SGC"
        If Tp = 1 Then
            GravarDados
        ...
        End If
    Case 1
        mobjMsg.Abrir "Deseja sair da tela de cadastro de Avaliações?", YesNo, pergunta, "SGC"
        If Tp = 1 Then
            Unload Me
            Set frmAvaliacoes = Nothing
        End If
...
Private Function ValidaCampo()
    ...
    If txtCadAvaliacao(0).Text = "" Then
        mobjMsg.Abrir "Favor informar o campo " & Me.txtCadAvaliacao(0).Tag, Ok, critico, "Atenção"
        Me.txtCadAvaliacao(0).SetFocus
        Exit Function
    End If
    If txtCadAvaliacao(1).Text = "" Then
        mobjMsg.Abrir "Favor informar o campo " & Me.txtCadAvaliacao(1).Tag, Ok, critico, "Atenção"
        Me.txtCadAvaliacao(1).SetFocus
        Exit Function
    End If
    If txtCadAvaliacao(2).Text = "" Then
        mobjMsg.Abrir "Favor informar o campo " & Me.txtCadAvaliacao(2).Tag, Ok, critico, "Atenção"
        Me.txtCadAvaliacao(2).SetFocus
        Exit Function
    End If
    
    If cboCadAvaliacao.Text = "" Then
        mobjMsg.Abrir "Favor informar o campo " & Me.cboCadAvaliacao.Tag, Ok, critico, "Atenção"
...
Private Sub ResultPesq()
    ...
    If rsAvaliacoes.RecordCount > 0 Then
        CompoeControles
    Else
        mobjMsg.Abrir "Avaliação não encontrada", Ok, critico, "Atenção"
    End If
    rsAvaliacoes.Close
    Set rsAvaliacoes = Nothing
End Sub
