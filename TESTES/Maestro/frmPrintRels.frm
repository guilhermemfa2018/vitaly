...
Begin VB.Form frmPrintRels 
   ...
   Begin MAESTRO.chameleonButton cmdImprimir 
      ...
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "frmPrintRels.frx":3469A
      PICN            =   "frmPrintRels.frx":346B6
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
   ...
   Begin MAESTRO.chameleonButton cmdImprimir 
      ...
      MCOL            =   12632256
      MPTR            =   1
      MICON           =   "frmPrintRels.frx":35390
      PICN            =   "frmPrintRels.frx":353AC
      UMCOL           =   -1  'True
      SOFT            =   0   'False
      PICPOS          =   0
      NGREY           =   0   'False
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Height          =   315
         ItemData        =   "frmPrintRels.frx":36086
         Left            =   120
         List            =   "frmPrintRels.frx":36096
         TabIndex        =   1
        ...
        ElseIf apontaLV = 18 Then
            If Combo1.ListIndex = 0 Then
                If MeuLV.ListView1.SelectedItem.ListSubItems.Item(9) = "-" Then
                    mobjMsg.Abrir "ADP não avaliada, não pode ser impressa", Ok, critico, "Atenção"
                    Exit Sub
                End If
                If MeuLV.ListView1.ListItems.Count > 0 Then
                    ...
                    FCRADP.Show 1
                Else
                    mobjMsg.Abrir "Nenhuma Avaliação de Desempenho foi selecionada", Ok, critico, "SGC"
                End If
            End If
...
'        SqlPrintMatriz = "Insert into tbPrintHFunc(campo1,campo5) Values(" & vCPFColaborador & ",'FUNÇÕES' + space(89) + 'Período')"
        ...
        SqlPrintMatriz = "Insert into tbPrintHFunc(campo1,campo2,campo3,campo4,codcoligada) Select a.cpf,b.nometreinamento + ' (' + isnull(c.nomenivel,'') + ')',substring(b.cargahoraria,1,3) + ':'+ substring(b.cargahoraria,4,2),substring(convert(char,e.datainicio,103),1,10) + ' - ' + substring(convert(char,e.datafim,103),1,10) + '       ' + REPLICATE ('0',6 - len(d.codprogramacao))+convert(char,d.codprogramacao),'" & vCodcoligada & "' " & _
                         "From tbcolaboradorescur as a Inner join tbtreinamentos as b on a.codcoligada = '" & vCodcoligada & "' and a.codtreinamento = b.codtreinamento left join tbtreinamentosniv as c on a.codtreinamento = c.codtreinamento and  a.codnivel = c.codnivel left join tbpendentescur as d " & _
                         "on d.situacao = 'Aprovado' and d.cpf = a.cpf and d.codtreinamento = a.codtreinamento and d.codnivel = a.codnivel and d.codprogramacao is not null left join tbprogramacao as e on d.codprogramacao = e.codprogramacao where a.cpf = '" & vCPFColaborador & "'" & _
